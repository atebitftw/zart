...
void main() {
  group('ScreenModel Window Management', () {
    late ZScreenModel screen;

    setUp(() {
      screen = ZScreenModel(cols: 80, rows: 24);
    });

    test('Content Expansion: Window expands when content exceeds requested height', () {
    ...
    test('Smart Shrinking: Window shrinks when content is cleared', () {
      ...
      expect(screen.window1Grid.length, greaterThanOrEqualTo(5)); // Grid re-inited
    });

    test('Quote Box Suppression: Suppresses bracketed text when forced open', () {
    ...
    test('Quote Box Suppression: Does NOT suppress when request matches content', () {
