...
void main() {
  ...
  const int version = 3;
  const int programCounterAddress = 14297;
  ...
  group('Z Memory> ', () {
    ...
    test(
      ...
      () {
        expect(Z.engine.mem.loadw(Header.programCounterInitialValueAddr), equals(programCounterAddress));
      },
    );
    ...
    test("Z.machine.mem.loadw() correct reads a value from the global variables memory area with offset.", () {
      expect(Z.engine.mem.loadw(Z.engine.mem.globalVarsAddress + 8), equals(8101));
    });

    test("Z.machine.mem.readGlobal() correctly reads a value from the global variables memory area.", () {
      expect(Z.engine.mem.readGlobal(0x14), equals(8101));
    });

    test("Z.machine.mem.writeGlobal() correctly writes a value to the global variables memory area.", () {
