  ...
  group('Glulx Stack and Call Tests', () {
    ...
    test('stkcount', () async {
      ...
      final code = [
        ...
        // stkcount -> RAM[0x100]. Mode: Op1=F (RAM Any).
        // Op1=F. ModeByte=0x0F.
        // Addr=0x100.
        GlulxOpcodes.stkcount, 0x0F, 0x00, 0x00, 0x01, 0x00,
      ...
      expect(interpreter.memRead32(0x500), equals(3));
    });
    ...
    test('stkpeek', () async {
      ...
      final code = [
        ...
        // Push 20
        GlulxOpcodes.copy, 0x81, 20,
        // stkpeek 0 -> RAM[0x100] (Should be 20)
        // Pos=0 (Mode 1 - Const Byte 0). Dest=RAM[0x100] (Mode F - RAM Any).
        // Byte 1: Op1(1) | Op2(F)<<4 = 0xF1.
        // Op1 Value: 0. Op2 Value: 0x100.
        GlulxOpcodes.stkpeek, 0xF1, 0x00, 0x00, 0x00, 0x01, 0x00,
        // stkpeek 1 -> RAM[0x104] (Should be 10)
        GlulxOpcodes.stkpeek, 0xF1, 0x01, 0x00, 0x00, 0x01, 0x04,
        GlulxOpcodes.quit >> 8 & 0x7F | 0x80, GlulxOpcodes.quit & 0xFF,
      ];
      ...
      // ramStart is 0x100. Operand 0x100 -> 0x200.
      expect(interpreter.memRead32(0x500), equals(20));
      expect(interpreter.memRead32(0x504), equals(10));
    });
    ...
    test('stkswap', () async {
      final code = [
        ...
        // Pop (Top) -> RAM[0x100]. Should be 10.
        // copy stack(8) -> RAM(F). Mode 0xF8.
        GlulxOpcodes.copy, 0xF8, 0x00, 0x00, 0x01, 0x00,
        // Pop (Next) -> RAM[0x104]. Should be 20.
        GlulxOpcodes.copy, 0xF8, 0x00, 0x00, 0x01, 0x04,
        GlulxOpcodes.quit >> 8 & 0x7F | 0x80, GlulxOpcodes.quit & 0xFF,
      ];
      ...
      expect(interpreter.memRead32(0x500), equals(10));
      expect(interpreter.memRead32(0x504), equals(20));
    });
    ...
    test('stkroll', () async {
      ...
      final code = [
        ...
        // Pop 5 items and store
        GlulxOpcodes.copy, 0xF8, 0x00, 0x00, 0x01, 0x00, // Top
        GlulxOpcodes.copy, 0xF8, 0x00, 0x00, 0x01, 0x04,
        GlulxOpcodes.copy, 0xF8, 0x00, 0x00, 0x01, 0x08,
        GlulxOpcodes.copy, 0xF8, 0x00, 0x00, 0x01, 0x0C,
        GlulxOpcodes.copy, 0xF8, 0x00, 0x00, 0x01, 0x10, // Bottom
      ...
      expect(interpreter.memRead32(0x500), equals(0xB));
      expect(interpreter.memRead32(0x504), equals(0xA));
      expect(interpreter.memRead32(0x508), equals(0xE));
      expect(interpreter.memRead32(0x50C), equals(0xD));
      // Bottom is C (0xC)
      expect(interpreter.memRead32(0x510), equals(0xC));
    });
    ...
    test('call and ret', () async {
      ...
      final mainFunc = [
        ...
        // Op3: RAM[0x200] (Mode F - RAM Any).
        // Modes: Op1(2), Op2(1) -> 0x12.
        // Byte 2: Op3(F) -> 0x0F.
        GlulxOpcodes.call, 0x12, 0x0F,
        0x01, 0x00, // Address 0x100
        0, // 0 args (Byte const)
        0x00, 0x00, 0x02, 0x00, // Dest 0x200
      ...
      expect(interpreter.memRead32(0x1200), equals(42));
    });
    ...
    test('call with arguments', () async {
      final mainFunc = [
        ...
        // Op1(0x100) Mode 2. Op2(2) Mode 1. Op3(RAM 0x200) Mode F.
      ...
      final calledFunc = [
        0xC0, 0x00, 0x00, // Header
        // Discard ArgCount (Top of stack)
        // copy Stack(8) -> Discard(0). Mode 0x08.
        GlulxOpcodes.copy, 0x08,

        // Stack: 20 (Top), 10.
        // add Stack(8) Stack(8) -> Stack(8)
        // Modes: 8, 8, 8.
        // Byte 1: 88. Byte 2: 08.
      ...
      expect(interpreter.memRead32(0x1200), equals(30));
    });
  });
}
