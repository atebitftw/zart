...
class InterpreterV3 {
  ...
  void branch(bool testResult) {
    ...
    if (BinaryHelper.isSet(jumpByte, 7) == testResult) {
      ...
      // operation.
      if (offset == InterpreterV3.gameFalse || offset == InterpreterV3.gameTrue) {
        doReturn(offset);
        return;
      }
  ...
  Future<void> read() async {
    ...
    // Traditional mode
    final l = await Z.sendIO({"command": ZIoCommands.read});
    log.finest("read() processing input");
    processLine(l);
    log.fine("pc: $programCounter");
  }
  ...
  void inc() {
    ...
    // For variable-type operands, we dereference to get the target
    final targetVarNum = operand.oType == OperandType.variable ? readVariable(operand.rawValue!) : operand.rawValue!;
  ...
  void quit() async {
    ...
    Z.inInterrupt = true;
    await Z.sendIO({"command": ZIoCommands.print, "window": currentWindow, "buffer": Z.sbuff.toString()});

    Z.inInterrupt = false;
    Z.sbuff.clear();
    Z.quit = true;

    await Z.sendIO({"command": ZIoCommands.quit});
  }
  ...
  void visitHeader() {
    ...
    final (screenWidth, screenHeight) = Z.io.getScreenSize();
    final cappedHeight = screenHeight > 254 ? 254 : screenHeight;
    final cappedWidth = screenWidth > 255 ? 255 : screenWidth;

    mem.storeb(Header.screenHeight, cappedHeight);
    mem.storeb(Header.screenWidth, cappedWidth);
    ...
    if (version.index >= ZMachineVersions.v5.index) {
      mem.storew(Header.screenWidthUnits, screenWidth); // 0x22: screen width in units
      mem.storew(Header.screenHeightUnits, screenHeight); // 0x24: screen height in units
