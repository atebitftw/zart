...
class GameObject {
  ...
  int getNextProperty(int? pnum) {
    ...
    // Property doesn't exist - return 0 for graceful error handling
    // This handles edge cases like operations on object 0
    if (addr == 0) {
      return 0;
    }
  ...
  static int getPropertyDefault(int propertyNum) {
    ...
    if (index < 0 || index >= maxProps) {
      throw GameException('property number out of bounds (1-$maxProps): $propertyNum');
    }
