...
///
/// This class uses platform-specific implementations:
/// - **Native (Dart VM/AOT)**: Simple `& 0xFFFFFFFF` bitmask - fast and direct
/// - **Web (dart2js)**: `Uint32List` to guarantee correct 32-bit unsigned semantics
class GlulxBinaryHelper {
  /// Detect if we're running on web by checking if 1 << 32 == 0
  /// In JavaScript, bit shifts are limited to 32 bits so 1 << 32 becomes 1.
  /// In native Dart, 1 << 32 is a large number.
  /// This is a compile-time constant that enables tree shaking.
  static final bool _isWeb = identical(1, 1.0);

  /// Shared buffer for 32-bit conversions (used only on web).
  ...
  static int shr32(int a, int shift) {
    ...
    // For native, ensure unsigned by masking first, then shift
    return ((a & _mask32) >> shift);
  }
