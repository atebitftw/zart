  ...
  void throwOp() {
    ...
    while (callStack.length > targetFrame) {
      ...
      if (callStack.length == targetFrame) {
        if (resultAddrByte != Engine.stackMarker) {
    ...
    switch (stream.abs()) {
      ...
      case 3:
        if (stream < 0) {
          ...
          //print('[output_stream -3] Writing "$capturedData" to addr=0x${addr.toRadixString(16)}');
          ...
          //print('[output_stream 3] Saving screen buffer: "$savedContent"');
